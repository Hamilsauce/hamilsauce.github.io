  ...
  arrOfNoteObjects.forEach(curr => {
    ...
    let props = entries
      .map(e => { //* for each prop value, embed in td markup string
        let val = e.trim();
        return `<td class="note-prop column${++i}">${val}</td>`;
      });
    i = 0;
...
let reduceAndAppendRows = (arr) => {
  ...
  arr.forEach(row => { //*for each array of tds in array
    ...
    document.querySelector('.tableBody').appendChild(tRow); //* slap that bad boy on the end of the table
...
(() => {
  ...
  inputVal.addEventListener('change', e => { //* replace existing table/markup/data w/ headers only (clear out old data)
    e.preventDefault();
